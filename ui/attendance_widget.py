...
from database.db import get_session, select, YogaClass, User, Attendance, Reserve, Role, Payment, PackageUsage
...
class AttendanceWidget(QWidget):
    ...
    def load_attendance_for_class(self):
        ...
        try:
            ...
            # Filtrar solo aquellos que han pagado (con dinero o paquete)
            paid_student_ids = []
            for sid in student_ids:
                ...
                usage = session.exec(
                    select(PackageUsage).where(
                        PackageUsage.yogaclass_id == class_id,
                        PackageUsage.student_package.has(student_id=sid)
                    )
                ).first()
                if payment or usage:
                    paid_student_ids.append(sid)

            # Obtener los datos de los estudiantes pagados
            if paid_student_ids:
                students = session.exec(
                    select(User).where(User.id.in_(paid_student_ids)).order_by(User.name)
                ).all()
            else:
                students = []
                self.class_info_label.setText("No hay alumnos con pago para esta clase")
                self.attendance_table.setRowCount(0)
                self.update_stats()
                return
            ...
            # Llenar la tabla con los estudiantes pagados
            ...
            QMessageBox.critical(self, "Error", f"Error al cargar asistencia: {str(e)}")
        finally:
            session.close()
    ...
    def save_attendance(self):
        ...
        try:
            ...
            self.load_attendance_for_class()
