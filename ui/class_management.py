...
class ClassManagementWidget(QWidget):
    ...
    def init_ui(self):
        ...
        # Botón para agregar clase (solo administradores/recepcionistas)
        ...
        # Botón para reservar clase (solo estudiantes)
        if self.current_user.role == Role.STUDENT:
            reserve_btn = QPushButton("📅 Reservar Clase")
            reserve_btn.clicked.connect(self.open_reservation_dialog)
            toolbar.addWidget(reserve_btn)
    ...
    def display_classes(self, classes):
        ...
        try:
            for row, yoga_class in enumerate(classes):
                self.classes_table.setItem(row, 0, QTableWidgetItem(str(yoga_class.id)))
                self.classes_table.setItem(
                    row,
                    1,
                    QTableWidgetItem(
                        yoga_class.scheduled_at.strftime("%Y-%m-%d %H:%M")
                    ),
                )
                ...
                self.classes_table.setItem(row, 5, QTableWidgetItem(str(available)))
    ...
    def open_reservation_dialog(self):
        """Abre el diálogo de reserva para estudiantes."""
        dialog = ClassReservationDialog(self.current_user)
        if dialog.exec():
            self.load_classes()
            ...
            try:
                ...
                if db_delete_class(class_id):
                    QMessageBox.information(
                        self, "Éxito", "Clase eliminada correctamente"
                    )
                    self.load_classes()
                else:
                ...
                QMessageBox.critical(
                    self, "Error", f"No se pudo eliminar la clase: {str(e)}"
                )
...
class AddClassDialog(QDialog):
    ...
    def load_teachers_and_centers(self):
        ...
        try:
            # Cargar profesores
            teachers = session.exec(select(User).where(User.role == Role.TEACHER)).all()
    ...
    def create_class(self):
        ...
        try:
            ...
            if yoga_class:
                ...
                QMessageBox.critical(self, "Error", "No se pudo crear la clase")
        except Exception as e:
